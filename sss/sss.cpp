...
extern "C" int sss_decrypt(uint8_t *out, uint8_t const* in, uint8_t const* key)
{
	...
	memcpy(&c[crypto_secretbox_BOXZEROBYTES],
	       in, sss_CLEN);
	int ret = crypto_secretbox_open(m, c, clen, nonce, key);
	memcpy(out, &m[crypto_secretbox_ZEROBYTES], sss_MLEN);
...
int sss_combine_shares(uint8_t *data, const sss_Share *shares, uint8_t k)
{
  ...
  return sss_decrypt(data, &shares[0][sss_KEYSHARE_LEN], key);
}
